...
DATASET = "CS"
SIMULATION_MODE = "subgraph_fl_metis_plus"
EPSILONS = [10]
CLIENT_COUNTS = [5, 10, 20]
SEEDS = [0, 1, 2]
RUNS_DIR = "runs"
JSON_PATH = os.path.join(RUNS_DIR, "table9_min_ablation.json")
CSV_PATH = os.path.join(RUNS_DIR, "table9_min_ablation.csv")
LOG_PATH = os.path.join(RUNS_DIR, "table9_min_ablation.log")
QUIET_TRAINER = True
...
def run():
    ...
    logging.info("Starting table9 minimal ablation run")
    ...
    results = {}
    total_runs = len(EPSILONS) * len(CLIENT_COUNTS) * len(VARIANTS) * len(SEEDS)
    run_idx = 0

    with tqdm(total=total_runs, desc="Table9 Min Ablation", unit="run") as pbar:
        for client_count in CLIENT_COUNTS:
            args.num_clients = client_count
            results.setdefault(client_count, {})

            for eps in EPSILONS:
                args.dp_eps = eps
                results[client_count].setdefault(eps, {})
                ...
                for variant in VARIANTS:
                    ...
                    for seed in SEEDS:
                        ...
                        logging.info(
                            "Run %d/%d start: %s clients=%s eps=%s seed=%s",
                            run_idx,
                            total_runs,
                            name,
                            client_count,
                        ...
                        logging.info(
                            "Run %d/%d end: %s clients=%s eps=%s seed=%s acc=%.6f time=%.2fs",
                            run_idx,
                            total_runs,
                            name,
                            client_count,
                        ...
                        _append_csv_row(client_count, eps, name, seed, best_acc, duration)
                        pbar.update(1)

                    mean_acc, std_acc = _mean_std(accs)
                    results[client_count][eps][name] = {
                        "mean": mean_acc,
                        "std": std_acc,
                        "seeds": accs,
                    }
    ...
    logging.info("Completed table9 minimal ablation run")
...
def _append_csv_row(client_count, eps, variant, seed, acc, duration):
    new_file = not os.path.exists(CSV_PATH)
    with open(CSV_PATH, "a", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        if new_file:
            writer.writerow(["dataset", "simulation", "epsilon", "num_clients", "variant", "seed", "best_test_accuracy", "duration_sec"])
        writer.writerow([DATASET, SIMULATION_MODE, eps, client_count, variant, seed, f"{acc:.6f}", f"{duration:.2f}"])
